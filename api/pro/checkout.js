...
// Body: { user_id, email? }
// Crée une session Stripe Checkout (abonnement mensuel) et renvoie l'URL de
// redirection. Le user_id Supabase est propagé dans les métadonnées de la
// session ET de l'abonnement → le webhook sait à qui accorder le Pro.
...
export default async (req) => {
  ...
  if (!key || !price) return Response.json({ error: 'stripe_non_configure' }, { status: 501 });
