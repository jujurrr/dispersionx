...
// Retourne Greeks ATM straddle portfolio + scénarios via Black-Scholes
export const config = { runtime: 'edge' };

const R = 0.043; // taux sans risque
...
// Greeks ATM straddle pour 1 straddle (1 call + 1 put)
function bsAtm(S, sigma, T) {
  ...
  return {
    // Premium approximatif ATM straddle
    premium: 2 * S * (normCDF(d1) - 0.5) + S * sigma * sqrtT * normPDF(d1) * 0.5,
    gamma:   2 * nd1 / (S * sigma * sqrtT),
    vega:    2 * S * nd1 * sqrtT / 100,   // par 1% IV
    theta: -(S * sigma * nd1) / (2 * sqrtT * 365), // par jour
  };
}
...
async function fetchIVFromMD(sym, dte, token) {
  ...
  try {
    ...
    // ATM = index le plus proche du prix sous-jacent
...
export default async (req) => {
  ...
  const SCALE = { SPX: 10, NDX: 41, DJI: 100, CAC: 196, DAX: 565 };
  const idxEtf = ETF[indexSym] || indexSym;
  const scale  = SCALE[indexSym] || 1;
  const mdTok  = process.env.MARKETDATA_API_TOKEN;
  const T = duration / 365;

  // Fetch prix + HV + IV en parallèle pour chaque ticker
  ...
  const perTicker = tickers.map((t, i) => {
    ...
    const iv    = ivMD ?? (hv ? hv * 1.15 : null); // IV ≈ HV × 1.15 si pas de données options
  ...
  // Greeks par ticker (long straddle)
  ...
  // Greeks indice (short straddle sur l'ETF proxy)
  const idxCloseReal = indexClose;
  const idxG = idxIVfn && idxCloseReal ? bsAtm(idxCloseReal, idxIVfn / 100, T) : null;
  const idxVega   = idxG ? -(idxG.vega   * CONTRACT) : 0;
  const idxTheta  = idxG ? -(idxG.theta  * CONTRACT) : 0;
  const idxPremium= idxG ?  (idxG.premium * CONTRACT) : 0; // crédit reçu

  const netVega   = Number((portVega   + idxVega).toFixed(2));
  const netTheta  = Number((portTheta  + idxTheta).toFixed(2));
  const netPremium= Number((portPremium - idxPremium).toFixed(2));
  ...
  const pnlByName = perTickerResult.map((r) => ({
    t: r.ticker,
    pnl: r.greeks ? Math.round(r.greeks.vega * 15 * CONTRACT * 0.5) : 0,
  }));
  ...
  return Response.json({
    ...
    pnlBySector,
    portfolio: {
      n_tickers: perTicker.length,
      avg_iv:    avgIV,
      index_iv:  idxIVfn ? Number(idxIVfn.toFixed(1)) : null,
      net_vega:  netVega,
      net_theta: netTheta,
