...
// Icônes œil (Feather) — affichent/masquent le mot de passe.
const EyeIcon = (
...
function PwField({ value, onChange, placeholder, autoComplete, style }) {
  const [show, setShow] = React.useState(false);
  return (
    <div style={{ position: 'relative' }}>
      <input style={{ ...style, paddingRight: 44 }} type={show ? 'text' : 'password'} value={value} onChange={onChange} placeholder={placeholder} autoComplete={autoComplete} />
      <button type="button" onClick={() => setShow(s => !s)}
        aria-label={show ? 'Masquer le mot de passe' : 'Afficher le mot de passe'}
        title={show ? 'Masquer' : 'Afficher'}
        style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', padding: 6, cursor: 'pointer', color: 'var(--text-muted)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  async function submit(e) {
    ...
    if (C && C.configured && C.auth) {
      ...
      try {
        ...
        // La double authentification (si activée) est gérée par la PORTE MFA GLOBALE
        // (composant MfaGate dans app.jsx), déclenchée par l'événement dx-mfa-required
        // que src/cloud.js émet dès qu'une session AAL1 a un facteur vérifié — le code
        // est ainsi demandé quel que soit le mode d'entrée (mot de passe, OAuth,
        // rechargement), et non plus seulement ici.
