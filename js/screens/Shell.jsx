...
function Sidebar({ active, onNav, lists, user, pro }) {
  ...
  const navGroups = [...NAV, { group: 'Pro', items: [
    ...
    { id: 'corr-pro', label: 'Corrélation Pro', icon: 'corr', locked: !pro },
  ] }];
...
// Logique horaires de marché : source unique testable dans js/lib/market-hours.js
// (chargée avant en script classique → window.DXMarket). Voir les tests Node.
...
function MarketStatus({ apiOn }) {
  ...
  const M = window.DXMarket || { EXCHANGES: [], isExchangeOpen: () => false };
  const statuses = M.EXCHANGES.map(ex => ({ ...ex, isOpen: M.isExchangeOpen(ex, now) }));
