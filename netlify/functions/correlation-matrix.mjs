// Netlify Function: POST /api/correlation/matrix
...
export const config = { path: '/api/correlation/matrix' };

import { proxyEtf } from '../../api/_lib/proxy-scale.js';
...
// Volatilité historique annualisée (en décimal, ex: 0.25 = 25%)
...
// Formule fondamentale de la dispersion : la corrélation implicite est la valeur
// qui, avec les volatilités individuelles, explique la volatilité de l'indice.
// σ_indice_impl = VIX/100 (options marché réelles) ou HV_indice * 1.30 (proxy si VIX absent)
// σ̄_comp_impl  = moyenne HV composants * 1.08 (les IV individuelles excèdent légèrement les HV)
...
export default async (req) => {
  ...
  // VIX / indice de vol implicite selon l'indice
  const VOL_TICKER = { SPX: '^VIX', NDX: '^VXN', DJI: '^VIX' };
  const vixSym = VOL_TICKER[indexSym] || null; // null pour indices européens

  // Fetch en parallèle : tickers + ETF indice + VIX (si disponible)
  ...
  // Closes brutes et log-returns pour les prix ; closes brutes pour le VIX
  const closesRaw = {};
  const rets = {};
  ...
  // ρ réalisée = moyenne hors-diagonale de la matrice
  ...
  // σ_indice_impl : VIX réel si disponible, sinon HV indice × prime de vol typique (1.30)
  const hvIdx = rets[idxEtf] ? computeHV(rets[idxEtf]) : null;
  const sigmaIdxImpl = lastVix ?? (hvIdx ? hvIdx * 1.30 : null);

  // σ̄ composants = moyenne HV des tickers valides
  ...
  const rhoImpl = computeRhoImpl(sigmaIdxImpl, avgHvComp)
    ?? Math.min(0.92, rhoReal + 0.08); // fallback conservateur si données manquantes
  ...
  const history = Array.from({ length: 8 }, (_, w) => {
    ...
    // σ indice pour la fenêtre : VIX moyen de la fenêtre, ou HV rolling de l'ETF × 1.30
