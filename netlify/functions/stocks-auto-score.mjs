// Netlify Function: POST /api/stocks/auto-score
...
const R = 0.043;
const RHO_IMPL_EST = 0.65; // ρ implicite estimée de marché
...
function bsAtm(S, sigma, T) {
  ...
  return {
    ...
    delta:   0.02,  // ATM straddle delta ≈ 0
  };
}

// Barres Yahoo Finance 3 mois → { lastClose, hv30, rets, hvMin, hvMax }
async function fetchBarsData(sym) {
  try {
    ...
    // HV30 = annualisée sur les 30 derniers rendements
    ...
    // Estimation HV min/max annuel (proxy pour IV rank)
    const windowedHVs = [];
...
export default async (req) => {
  ...
  // Fetch en parallèle : barres stock + barres indice + IV MarketData
  ...
  // IV Rank (estimation depuis HV historique)
  ...
  const ivRank = ivMax > ivMin
    ? Math.round(Math.max(0, Math.min(100, (iv - ivMin) / (ivMax - ivMin) * 100)))
    : 50;
  const ivPct  = Math.round(Math.max(0, Math.min(100, ivRank * 0.95)));

  // ─── Composantes du score ────────────────────────────────────────
  const volPrem   = iv - hv30;                                           // B: IV − HV (en pts %)
  const edgeRho   = (RHO_IMPL_EST - rho) * 40;                          // A: prime de corrélation
  const betaScore = Math.max(0, 12 - Math.abs(beta - 1.1) * 10);        // +0-12 si beta ~1
  const volContrib = Math.max(-25, Math.min(25, volPrem));               // plafonné ±25

  const rawScore  = 50 + volContrib + edgeRho + betaScore;
  ...
  const subscores = {
    vol_attractive: {
      score: Math.round(Math.max(0, Math.min(99, 50 + volPrem * 1.8))),
      reason: `IV ${iv.toFixed(1)}% vs HV ${hv30.toFixed(1)}% (prime ${volPrem >= 0 ? '+' : ''}${volPrem.toFixed(1)} pts)`,
    },
    dispersion_contrib: {
      score: Math.round(Math.max(0, Math.min(99, (1 - Math.max(0, rho)) * 120))),
      reason: `ρ réalisée ${(rho * 100).toFixed(0)}% vs indice (${rho < 0.5 ? 'faible = favorable' : rho < 0.7 ? 'modérée' : 'élevée = défavorable'})`,
    },
    liquidity: {
      score: Math.min(99, Math.round(50 + Math.log(Math.max(1, price)) * 5)),
      reason: `Prix ${price.toFixed(2)}$ — proxy liquidité (pas de données volume)`,
    },
    execution: {
      score: Math.min(99, Math.round(75 - rho * 28)),
      reason: `Spread estimé — qualité d'exécution dépend du beta et de la corrélation`,
    },
    event_risk: {
      score: 72,
      reason: 'Risque événement non disponible (calendrier earnings non intégré)',
    },
  };
  ...
  return Response.json({
    scoring: {
      ...
      signal,
      signal_color,
      comp_a_edge:           Number(edgeRho.toFixed(1)),
      comp_b_vol_premium:    Number(volPrem.toFixed(1)),
      comp_c_costs:          -3.8,
      rho_implicit_final:    RHO_IMPL_EST,
      rho_real_expected:     rho,
      cost_source:           'estimated',
      spread_pct_real:       0.20,
      cost_spread:           2.4,
      cost_earnings:         1.4,
      subscores,
      composite_score:       { score },
      pipeline: {
        rho_per_window:     { 20: Number((rho + 0.02).toFixed(3)), 60: rho, 120: Number((rho - 0.01).toFixed(3)) },
    ...
    stock: {
      symbol:                sym,
      weight:                Number((100 / 10).toFixed(1)),
      iv,
      hv:                    hv30,
      beta,
      last_price:            Number(price.toFixed(2)),
      iv_source:             ivSrc,
      earnings_in_strategy:  false,
      days_to_earnings:      45,
      earnings_date:         '—',
      iv_rank: {
        iv_rank:             ivRank,
        ...
        iv_min:              ivMin,
        iv_max:              ivMax,
        note:                ivMD ? 'IV réelle MarketData — rang estimé depuis HV historique' : 'IV et rang estimés depuis la HV historique Yahoo Finance',
      },
      greeks: g ? {
        delta:   Number(g.delta.toFixed(3)),
        gamma:   Number(g.gamma.toFixed(5)),
        vega:    Number(g.vega.toFixed(1)),
        theta:   Number(g.theta.toFixed(1)),
        strike:  Number(price.toFixed(0)),
        expiry:  expiryDate,
      } : null,
    },
    index: {
      symbol: indexSym,
      name:   { SPX: 'S&P 500', NDX: 'Nasdaq 100', DJI: 'Dow Jones', CAC: 'CAC 40', DAX: 'DAX' }[indexSym] || indexSym,
      iv:     idxData?.hv30 ? Number((idxData.hv30 * 1.1).toFixed(1)) : null,
    },
