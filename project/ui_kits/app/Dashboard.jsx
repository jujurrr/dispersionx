  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 14 }}>
      <div>
        ...
        {sub && <p style={{ font: 'var(--type-body-sm)', color: 'var(--text-muted)', margin: '4px 0 0' }}>{sub}</p>}
      </div>
      {action}
    </div>
  );
...
function Dashboard({ mode }) {
  ...
  const { MetricCard, ScoreBadge, RiskBadge, Button, BeginnerExplanationBox, Badge } = window.DispersionXDesignSystem_cb86be;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div>
        ...
        <p style={{ font: 'var(--type-body)', color: 'var(--text-muted)', margin: 0 }}>Lecture du jour : la prime de corrélation reste positive sur le SPX. Un signal à analyser, pas une recommandation.</p>
      </div>

      {/* Market Overview */}
      <section>
        <SectionHead title="Market Overview" sub="Volatilité et corrélation agrégées du marché" />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12 }}>
          {D.market.map((m, i) => <MetricCard key={m.label} {...m} glass style={{ animationDelay: (i * 70) + 'ms' }} />)}
        </div>
      </section>

      {/* Opportunités */}
      <section>
        <SectionHead title="Opportunités" sub="Indices où l'écart vol/corrélation mérite un examen" action={<Button variant="ghost" size="sm">Tout voir</Button>} />
            ...
            <tbody>
              {D.opportunities.map((o) => (
                <tr key={o.idx} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}><div style={{ display: 'inline-block' }}><ScoreBadge score={o.score} max={0} label="" /></div></td>
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}><RiskBadge level={o.risk} size="sm" /></td>
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}><Button variant="outline" size="sm">Analyser</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
        {/* Stratégies suivies */}
        <section>
          <SectionHead title="Stratégies suivies" />
        ...
        <section>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: i < 3 ? '1px solid var(--border-subtle)' : 'none' }}>
      ...
      {mode === 'Débutant' && (
        <section>
          <SectionHead title="Comprendre le signal du jour" />
...
Object.assign(window, { Dashboard, SectionHead });
